...
create policy if not exists "card_ingestion_images_select_admin" on storage.objects
  for select using (
...
create policy if not exists "card_ingestion_images_insert_admin" on storage.objects
...
create policy if not exists "card_ingestion_images_delete_admin" on storage.objects
  for delete using (
    bucket_id = 'card-ingestion-images'
    and auth.uid()::text = (storage.foldername(name))[2]
  );
