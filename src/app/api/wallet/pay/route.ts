...
async function getUser() {
  ...
  const supabase = createServerClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, { cookies: { getAll: () => store.getAll(), setAll: () => {} } })
...
export async function POST(req: Request) {
  try {
    ...
    const { data: balance, error: debitError } = await admin.rpc('wallet_debit_customer', { p_user_id: user.id, p_amount: total, p_reference_id: listingId, p_idempotency_key: key })
    if (debitError) {
    ...
    if (orderError) {
      await admin.rpc('wallet_credit_customer', { p_user_id: user.id, p_amount: total, p_reference_id: listingId, p_idempotency_key: `${key}-rollback` })
