    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(255,171,1,0.12),_transparent_28%),linear-gradient(180deg,#0f0f1a_0%,#090b14_100%)] text-white">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] shadow-2xl shadow-black/20">
          <div className="grid gap-6 px-5 py-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:px-8 lg:py-8">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.35em] text-yellow-300">
                <span className="rounded-full border border-yellow-400/20 bg-yellow-400/10 px-3 py-1">Card lookup</span>
                <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-gray-300">Price history</span>
              </div>
              <h1 className="text-4xl font-black sm:text-5xl">Search a Pokémon card to see live pricing, official artwork, and recent market samples.</h1>
              <p className="max-w-2xl text-sm leading-6 text-gray-300 sm:text-base">Use the lookup to compare values, explore artwork, and save cards to your personal lists in a cleaner collector view.</p>
            </div>
            <div className="flex flex-wrap gap-3 lg:justify-end">
              <a href="/collection" className="rounded-2xl border border-white/10 px-4 py-3 text-sm font-semibold text-gray-200 transition hover:bg-white/5">My library</a>
        ...
        <form onSubmit={handleSearch} className="mt-6 grid gap-3 rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4 md:grid-cols-5">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-4">
            {cardName.trim() && (
              <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-4">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="text-sm font-semibold uppercase tracking-widest text-yellow-300">Suggestions</h2>
                ...
                {suggestionsLoading ? (
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    {suggestions.map((card) => (
                      <button key={card.id} type="button" onClick={() => { setCardName(card.name); setSetName(card.setName); setSelectedCard(card); }} className={`rounded-xl border px-4 py-3 text-left transition-colors ${activeSuggestion?.id === card.id ? "border-yellow-400 bg-yellow-400/10" : "border-white/10 bg-[#13131f] hover:border-white/20"}`}>
            ...
            {price && (
              <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-6">
                <div className="text-sm uppercase tracking-widest text-yellow-300">{price.source}</div>
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-xl border border-white/10 bg-[#13131f] p-4"><div className="text-xs uppercase tracking-widest text-gray-500">Market</div><div className="mt-2 text-2xl font-black">{price.marketPrice !== null ? `$${price.marketPrice.toFixed(2)}` : "—"}</div></div>
                  <div className="rounded-xl border border-white/10 bg-[#13131f] p-4"><div className="text-xs uppercase tracking-widest text-gray-500">Low</div><div className="mt-2 text-2xl font-black">{price.lowPrice !== null ? `$${price.lowPrice.toFixed(2)}` : "—"}</div></div>
                  <div className="rounded-xl border border-white/10 bg-[#13131f] p-4"><div className="text-xs uppercase tracking-widest text-gray-500">High</div><div className="mt-2 text-2xl font-black">{price.highPrice !== null ? `$${price.highPrice.toFixed(2)}` : "—"}</div></div>
                </div>
                {!isSignedIn && (
                  <div className="mt-4 rounded-xl border border-white/10 bg-[#13131f] p-4 text-sm text-gray-300">
                    ...
                    <a href="/auth" className="mt-3 inline-flex rounded-lg bg-yellow-400 px-3 py-2 text-sm font-semibold text-black hover:bg-yellow-300">Sign in now</a>
                  </div>
                )}
                <button type="button" onClick={() => void handleSaveCard()} disabled={!isSignedIn} className="mt-4 rounded-xl border border-yellow-400/30 bg-yellow-400/10 px-4 py-3 text-sm font-semibold text-yellow-300 hover:bg-yellow-400/20 disabled:cursor-not-allowed disabled:opacity-50">Save to {SAVE_TARGETS.find((item) => item.key === saveTarget)?.label}</button>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.04]">
              <div className="border-b border-white/10 px-4 py-3"><h2 className="text-sm font-semibold uppercase tracking-widest text-yellow-300">Official artwork</h2></div>
              <div className="flex min-h-[360px] items-center justify-center bg-[#13131f] p-4">
                {activeSuggestion?.image ? (
                  ...
                  <div className="text-center text-gray-500"><div className="text-5xl">🃏</div><p className="mt-3 text-sm">Select a card suggestion to preview artwork.</p></div>
                )}
              </div>
            </div>

            <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.04] p-6">
              <div className="flex items-center justify-between gap-3"><h2 className="text-sm font-semibold uppercase tracking-widest text-yellow-300">Recent price history</h2><span className="text-xs text-gray-500">{hasHistory ? `${history.length} saved samples` : "No samples yet"}</span></div>
              {history.length ? (
                <>
                  <div className="mt-4 flex h-28 items-end gap-2 rounded-xl border border-white/10 bg-[#13131f] p-3">
                    {chartPoints.map((point, index) => (<div key={`${point.value}-${index}`} className="flex flex-1 flex-col items-center justify-end"><div className="w-full rounded-t-md bg-yellow-400/80" style={{ height: `${point.height}%` }} /></div>))}
                  </div>
                  <div className="mt-4 space-y-3">
                    {history.map((entry) => (
                      <div key={entry.id} className="rounded-xl border border-white/10 bg-[#13131f] px-4 py-3 text-sm text-gray-300">
                        <div className="flex items-center justify-between gap-3"><div className="font-semibold text-white">${entry.price.toFixed(2)}</div><div className="text-xs text-gray-500">{entry.source}</div></div>
