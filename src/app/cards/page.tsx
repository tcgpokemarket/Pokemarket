      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 sm:p-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <div className="text-xs uppercase tracking-[0.3em] text-yellow-400">Card lookup</div>
              <h1 className="mt-2 text-3xl font-black sm:text-4xl">Search, price, and save cards</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-gray-400">Look up a Pokémon card, compare recent market samples, and save it to your collection, wishlist, or deck.</p>
            </div>
            <div className="flex gap-3">
              <a href="/collection" className="rounded-full border border-white/10 px-4 py-2.5 text-sm font-semibold text-gray-300 hover:border-yellow-400/40 hover:text-yellow-300">My library</a>
              <a href="/listings" className="rounded-full bg-yellow-400 px-4 py-2.5 text-sm font-bold text-black hover:bg-yellow-300">Browse listings</a>
            </div>
          </div>
        </div>

        <form onSubmit={handleSearch} className="grid gap-3 rounded-[2rem] border border-white/10 bg-[#101724] p-4 shadow-xl shadow-black/20 md:grid-cols-5">
          <input value={cardName} onChange={(e) => setCardName(e.target.value)} placeholder="Card name" className="rounded-2xl border border-white/10 bg-[#13131f] px-4 py-3 text-white outline-none focus:border-yellow-400 md:col-span-2" />
          <input value={setName} onChange={(e) => setSetName(e.target.value)} placeholder="Set name" className="rounded-2xl border border-white/10 bg-[#13131f] px-4 py-3 text-white outline-none focus:border-yellow-400 md:col-span-1" />
          <select value={saveTarget} onChange={(e) => setSaveTarget(e.target.value as SaveTarget)} className="rounded-2xl border border-white/10 bg-[#13131f] px-4 py-3 text-white outline-none focus:border-yellow-400 md:col-span-1">
            {SAVE_TARGETS.map((target) => <option key={target.key} value={target.key}>{target.label}</option>)}
          </select>
          <button type="submit" disabled={loading} className="rounded-2xl bg-yellow-400 px-4 py-3 font-bold text-black hover:bg-yellow-300 disabled:opacity-50 md:col-span-1">{loading ? "Searching..." : "Search"}</button>
        </form>

        <div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
          <div className="space-y-4">
            {cardName.trim() && (
              <section className="rounded-[2rem] border border-white/10 bg-[#101724] p-5 shadow-xl shadow-black/20">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="text-sm font-semibold uppercase tracking-[0.25em] text-yellow-400">Suggestions</h2>
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    {suggestions.map((card) => (
                      <button
                        ...
                        className={`rounded-2xl border px-4 py-3 text-left transition-colors ${activeSuggestion?.id === card.id ? "border-yellow-400 bg-yellow-400/10" : "border-white/10 bg-[#13131f] hover:border-white/20"}`}
                      >
            ...
            {price && (
              <section className="rounded-[2rem] border border-white/10 bg-[#101724] p-5 shadow-xl shadow-black/20">
                <div className="text-xs uppercase tracking-[0.3em] text-yellow-400">{price.source}</div>
                ...
                {!isSignedIn && (
                  <div className="mt-4 rounded-2xl border border-white/10 bg-[#13131f] p-4 text-sm text-gray-300">
                    ...
                    <a href="/auth" className="mt-3 inline-flex rounded-full bg-yellow-400 px-3 py-2 text-sm font-semibold text-black hover:bg-yellow-300">
                      Sign in now
                    </a>
                  </div>
                )}
                <button
                  ...
                  disabled={!isSignedIn}
                  className="mt-4 rounded-full border border-yellow-400/30 bg-yellow-400/10 px-4 py-3 text-sm font-semibold text-yellow-400 hover:bg-yellow-400/20 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Save to {SAVE_TARGETS.find((item) => item.key === saveTarget)?.label}
                </button>
                {savedMessage && <div className="mt-3 rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-200">{savedMessage}</div>}
              </section>
            )}
          </div>

          <div className="space-y-4">
            <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#101724] shadow-xl shadow-black/20">
            ...
            <section className="rounded-[2rem] border border-white/10 bg-[#101724] p-5 shadow-xl shadow-black/20">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-gray-500">{visibleHistory.length ? `${history.length} saved samples` : "No samples yet"}</span>
              </div>
              {history.length ? (
                <>
                  <div className="mt-4 flex h-28 items-end gap-2 rounded-2xl border border-white/10 bg-[#13131f] p-3">
                  ...
                  <div className="mt-4 space-y-3">
                    {history.map((entry) => (
                      <div key={entry.id} className="rounded-2xl border border-white/10 bg-[#13131f] px-4 py-3 text-sm text-gray-300">
                        <div className="flex items-center justify-between gap-3">
