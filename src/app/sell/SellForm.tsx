...
export default function SellForm(){
 async function submit(e:React.FormEvent<HTMLFormElement>){
  ...
  if(!user) return
  await supabase.from('listings').insert([{seller_id:user.id,title:String(form.get('title') ?? ''),price:Number(form.get('price') ?? 0),description:String(form.get('description') ?? '')}] as any)
 }
