...
async function getListing(id: string) {
  const rows = await fetchPublicRows<ListingWithSeller>(
    "listings",
    "*,profiles:profiles!seller_id(id,username,seller_rating,total_sales,avatar_url,verification_status)",
    [["id", `eq.${id}`]],
  );
  return rows[0] ?? null;
}
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: canonical, type: "website", images: [{ url: image, width: 1200, height: 1200, alt: title }] },
...
export default async function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const listing = await getListing(id);
  if (!listing) return <div className="min-h-screen bg-[#0f0f1a] text-white flex items-center justify-center"><h1 className="text-3xl font-black">Listing not found</h1></div>;
  ...
  const jsonLd = {
    ...
    image: listing.images?.length ? listing.images : [image], sku: listing.card_number ?? listing.id,
    brand: { "@type": "Brand", name: "Pokémon TCG" }, category: listing.category,
    offers: { "@type": "Offer", url: canonical, priceCurrency: "USD", price: listing.price.toFixed(2), availability: listing.status === "active" ? "https://schema.org/InStock" : "https://schema.org/OutOfStock", itemCondition: "https://schema.org/UsedCondition" },
    seller: listing.profiles?.username ? { "@type": "Person", name: listing.profiles.username } : undefined,
  };
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><ListingDetailClient id={listing.id} initialListing={listing} /></>;
}
