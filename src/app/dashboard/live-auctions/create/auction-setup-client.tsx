...
export default function AuctionSetupClient({ sellerName, sellerUsername, listings, existingShows, currentUserId }: AuctionSetupClientProps) {
  ...
  const launchBlockedMessage = currentLiveShow ? `You already have an active live show. End your current show before starting another.` : null;
  const launchDisabled = Boolean(currentLiveShow) || !canLaunchLive;
  const activeShowHref = currentLiveShow ? `/live/${currentLiveShow.id}` : "/live";
  const activeShowLabel = currentLiveShow ? `Open ${currentLiveShow.title}` : "Open live show";
            ...
            <div className="rounded-3xl border border-yellow-400/20 bg-yellow-400/10 p-6">
              ...
              {launchBlockedMessage && (
                <div className="mt-4 rounded-2xl border border-red-400/30 bg-red-400/10 p-4 text-sm text-red-100">
                  <div className="font-semibold">Active live show</div>
                  <p className="mt-1">{launchBlockedMessage}</p>
                  {currentLiveShow && (
                    <a href={activeShowHref} className="mt-3 inline-flex rounded-xl border border-red-300/40 px-4 py-2 font-semibold text-red-100 hover:bg-red-400/10">
                      {activeShowLabel}
                    </a>
                  )}
                </div>
