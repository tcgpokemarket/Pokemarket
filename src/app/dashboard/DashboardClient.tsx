...
const SUPPORT_CARD = (
  <SupportInlineCard title="Need help?" description="Get help with listings, fees, payouts, live shows, or seller tools." href="/help" />
);
...
export default function DashboardClient({ orderSuccess }: { orderSuccess: boolean }) {
  ...
  const [liveShowDescription, setLiveShowDescription] = useState("Add a description for this live show.");
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#0f0f1a] px-4 text-center text-gray-400">
        Loading dashboard...
      </div>
    );
  }
  ...
  const saveSellerAssets = async (updates: {
    ...
    if (updates.profile) {
      ...
      if ("seller_state" in updates.profile) {
  ...
  const renderUploadCard = (label: string, previewUrl: string | null, kind: "profileAvatar" | "profileBanner" | "sellerAvatar" | "sellerBanner" | "storeLogo" | "storeBanner") => (
    <div className="rounded-2xl border border-white/10 bg-[#13131f] p-3 sm:p-4">
      ...
      }} className="block w-full text-xs text-gray-300 file:mr-3 file:rounded-lg file:border-0 file:bg-yellow-400 file:px-3 file:py-2 file:text-xs file:font-semibold file:text-black hover:file:bg-yellow-300 sm:text-sm" />
      <div className="mt-3 aspect-[4/3] overflow-hidden rounded-xl border border-white/10 bg-black/20 sm:aspect-[16/9]">
        {previewUrl ? (
          <img src={previewUrl} alt={label} className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full items-center justify-center text-[11px] text-gray-500 sm:text-xs">No image yet</div>
        )}
      </div>
    </div>
  );
  ...
  const adminLinks = [
    ...
    { href: "/admin/card-ingestion", label: "Card Ingestion" },
    { href: "/admin/verification", label: "Verification" },
    { href: "/admin/referrals", label: "Referrals" },
    { href: "/admin/apis", label: "API Keys" },
  ];

  const canShowAdminSection = isAdminAccount;
  const adminSummary = isAdminAccount ? "Admin tools are available on this account." : "Admin tools are hidden on this account.";

  return (
    <main className="min-h-screen bg-[#0f0f1a] px-4 py-6 text-white sm:px-6 sm:py-10 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8 pb-8 lg:pb-0">
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
          <div className="space-y-4 rounded-[2rem] border border-white/10 bg-[#101724] p-5 shadow-xl shadow-black/20 sm:p-6">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-yellow-400/20 text-lg font-black text-yellow-400 sm:h-14 sm:w-14 sm:text-xl">
            ...
            <p className="max-w-2xl text-sm leading-6 text-gray-300 sm:text-lg sm:leading-relaxed">
              Manage listings, sales, fees, live rooms, and payouts from a cleaner mobile-first dashboard.
            </p>
              ...
              {!isAdminAccount && (
                <>
                  ...
                  {!isSellerVerificationApproved(verificationStatus) && (
                    <div className="mt-3 rounded-2xl border border-red-400/20 bg-red-400/10 p-3 text-xs leading-5 text-red-100 sm:mt-4 sm:p-4 sm:text-sm">
          ...
          <div className="space-y-3 rounded-[2rem] border border-white/10 bg-white/5 p-4 shadow-2xl shadow-black/20 backdrop-blur sm:space-y-4 sm:p-5">
            {SUPPORT_CARD}
            <div className="text-[11px] leading-5 text-gray-500 sm:text-xs">Support questions can also be sent from orders, live shows, or the help center.</div>
          </div>

          <div className="space-y-3 rounded-[2rem] border border-white/10 bg-white/5 p-4 shadow-2xl shadow-black/20 backdrop-blur sm:space-y-4 sm:p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.25em] text-yellow-400 sm:text-sm sm:tracking-widest">Quick action</p>
                <h2 className="mt-2 text-lg font-black sm:text-xl">Copy dashboard summary</h2>
              </div>
              <button onClick={handleCopyBrainSummary} className="rounded-full border border-yellow-400/40 bg-yellow-400/10 px-3 py-2 text-xs font-semibold text-yellow-400 transition-colors hover:bg-yellow-400/20 sm:px-4 sm:text-sm">
                {brainCopied ? "Copied" : "Copy"}
              </button>
            </div>
            <p className="text-xs leading-5 text-gray-400 sm:text-sm">Grab a clean summary of your store metrics for notes or planning.</p>
          ...
          {canShowAdminSection && (
            <div className="space-y-3 rounded-[2rem] border border-yellow-400/20 bg-yellow-400/10 p-4 shadow-2xl shadow-black/20 backdrop-blur sm:space-y-4 sm:p-5">
              <div>
                <p className="text-[11px] uppercase tracking-[0.25em] text-yellow-400 sm:text-sm sm:tracking-widest">Admin section</p>
                <h2 className="mt-2 text-lg font-black sm:text-xl">Marketplace admin tools</h2>
              </div>
              <p className="text-xs leading-5 text-gray-300 sm:text-sm">{adminSummary}</p>
              <div className="grid gap-2 sm:grid-cols-2">
                {adminLinks.map((link) => (
                  <a key={link.href} href={link.href} className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm font-semibold text-white hover:border-yellow-400/40 hover:bg-yellow-400/10">
                    {link.label}
                  </a>
                ))}
              </div>
        ...
        {profileAssetUploading && <div className="rounded-xl border border-yellow-400/20 bg-yellow-400/10 px-4 py-3 text-sm text-yellow-100">Uploading asset...</div>}

        <div className="flex gap-1 overflow-x-auto rounded-[2rem] border border-white/10 bg-[#101724] p-1">
          {TABS.map((t) => (
            <button
              key={t.key}
              onClick={() => setTab(t.key)}
              className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold transition-all ${tab === t.key ? "bg-yellow-400 text-black" : "text-gray-400 hover:text-white"}`}
            >
              {t.label}
            </button>
          ))}
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                <div key={stat.label} className="rounded-[1.75rem] border border-white/10 bg-white/5 p-5 text-center">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-[1.75rem] border border-white/10 bg-white/5 p-6">
                ...
                <button className="mt-4 rounded-full border border-yellow-400/30 bg-yellow-400/10 px-4 py-2 text-sm font-semibold text-yellow-400 hover:bg-yellow-400/20" disabled={!instantEligible}>
                  {instantEligible ? "Request instant payout" : "Instant payout locked"}
                </button>
              </div>

              <div className="rounded-[1.75rem] border border-yellow-400/20 bg-yellow-400/10 p-6">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <button onClick={() => copyReferralValue(referralLink, "Referral link")} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-left text-sm text-white hover:border-yellow-400/40">Copy referral link</button>
                  <button onClick={() => copyReferralValue(referralCode ?? "", "Referral code")} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-left text-sm text-white hover:border-yellow-400/40">Copy referral code</button>
                </div>
                {referralQrUrl && (
                  <div className="mt-4 flex items-center gap-4 rounded-2xl border border-white/10 bg-[#13131f] p-4">
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-[#13131f] px-4 py-3 text-xs text-gray-400">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-[1.75rem] border border-yellow-400/20 bg-gradient-to-br from-yellow-400/10 via-red-500/10 to-blue-500/10 p-6">
        ...
        {tab === "listings" && (
          <div>
            <div className="mb-5 flex items-center justify-between">
              ...
              <a href="/listings/create" className="rounded-full bg-yellow-400 px-4 py-2 text-sm font-bold text-black hover:bg-yellow-300">+ New Listing</a>
            </div>
            {listings.length === 0 ? (
              ...
              <div className="space-y-3">
                {listings.map((l) => (
                  <div key={l.id} className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 sm:flex-row sm:items-center">
                    <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white/10 text-2xl">
                    ...
                    <div key={show.id} className="rounded-2xl border border-yellow-400/20 bg-yellow-400/10 p-4">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2 lg:justify-end">
                          ...
                          <button className="rounded-xl border border-white/20 px-4 py-2 font-semibold text-white" onClick={() => router.push(`/help`)}>
                            Message team
                          </button>
                          <button className="rounded-xl border border-red-400/30 px-4 py-2 font-semibold text-red-200" onClick={() => router.push(`/help`)}>
                            Report issue
                          </button>
                        </div>
                      </div>
        ...
        {tab === "live" && (
          <div className="space-y-6 rounded-[2rem] border border-white/10 bg-[#101724] p-5 shadow-xl shadow-black/20">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              <a href="/dashboard/live-auctions/create" className="inline-flex rounded-full bg-yellow-400 px-4 py-3 text-sm font-bold text-black hover:bg-yellow-300">
                Create auction
              </a>
            </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <button
                    ...
                    className="rounded-full bg-yellow-400 px-4 py-3 font-bold text-black disabled:opacity-50"
                  >
                    {isSellerVerificationApproved(verificationStatus) ? "Create live room" : "Verification required"}
                  </button>
                  <button type="button" onClick={() => setLiveShowStatusMessage("Seller rooms load independently by show_id.")} className="rounded-full border border-white/20 px-4 py-3 text-gray-300 hover:bg-white/5">
                    Room isolation note
                  </button>
                </div>
