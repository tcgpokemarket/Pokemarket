...
const FOOTER_LINKS = [{ label: "Referrals", href: "/referrals" }, { label: "Email preferences", href: "/account/email-preferences" }];
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <section>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-relaxed text-gray-300">
              TCG Poke Market connects collectors, sellers, and live event hosts with a fast, secure marketplace built for Pokémon singles, sealed products, graded cards, and community-driven commerce.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="/listings" className="rounded-xl bg-yellow-400 px-5 py-3 font-bold text-black hover:bg-yellow-300">Shop listings</a>
              <a href="/sell" className="rounded-xl border border-white/20 px-5 py-3 font-semibold text-white hover:bg-white/5">Start selling</a>
            </div>

            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {highlights.map((item) => (
                <div key={item.title} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <div className="font-bold text-yellow-400">{item.title}</div>
          ...
          <aside className="rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8">
            <div className="text-sm font-semibold uppercase tracking-widest text-yellow-400">Explore the marketplace</div>
            <div className="mt-5 grid gap-3">
              {links.map((link) => (
                <a key={link.href} href={link.href} className="flex items-center justify-between rounded-2xl border border-white/10 bg-[#13131f] px-4 py-4 text-sm font-semibold text-white hover:border-yellow-400/40 hover:bg-[#171724]">
